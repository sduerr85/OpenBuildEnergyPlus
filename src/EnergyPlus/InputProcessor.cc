...
ProcessInput()
{
    ...
    EchoInputFile = GetNewUnitNumber();
    {
        IOFlags flags;
        flags.ACTION("write");
        gio::open(EchoInputFile, "eplusout.audit", flags);
    ...
    {
        IOFlags flags;
        gio::inquire("eplusout.iperr", flags);
    ...
    if (FileExists) {
        CacheIPErrorFile = GetNewUnitNumber();
        {
            IOFlags flags;
            flags.ACTION("read");
            gio::open(CacheIPErrorFile, "eplusout.iperr", flags);
            read_stat = flags.ios();
        }
        if (read_stat != 0) {
...
)
{
    ...
    {
        IOFlags flags;
        cross_platform_get_line(in_stream, InputLine);
        flags.set_status(in_stream);
        ReadStat = flags.ios();
    }
